...
In the repository, the UnsupervisedLearning directory contains all the necessary code
for this third assignment.

For this assignment I wrote indvidual python scripts to run each of the experiments and create the plots:
...
Each of the above scripts creates and saves named plots as pngs for each part of the assignment, description below
    ...
    kmeans.py:
        This script runs kmeans clustering with and without all the dimensionality reduction algorithms
        on the dataset given

    expecmax.py
        This script runs the GaussianMixtureModel clustering with and without all the dimensionality reduction 
        algorithms on the dataset given
    
    dimreduction.py
        This script runs the each dimensionality reduction algorithm on the dataset given

    neuralnet.py
        This script runs the neural network created with tensorflow on the given dataset with and without clustering
        on all the dimensionality reduction algorithms
...
Examples for running a script with the Phishing website dataset:
    ...
    python dimreduction.py d1 path_to_file/Training_Dataset.arff

    python expecmax.py d1 path_to_file/Training_Dataset.arff

Examples for running a script with the Census dataset:

    python kmeans.py d2 path_to_file/adult.data

    python neauralnet.py d2 path_to_file/adult.data