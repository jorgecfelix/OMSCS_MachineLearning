...
def get_validation_curve(file_name, dataset_to_use, dimreduc='', cluster=''):
    ...
    if cluster == 'kmeans':
        ...
        print(f"\n\n Before Clustering{X_train.shape}")
        kmeans = KMeans(n_clusters=n_clusters, random_state=1, algorithm='full').fit(X_train)
        add_train = pd.DataFrame(kmeans.predict(X_train))
        add_test = pd.DataFrame(kmeans.predict(X_test))

        X_train = np.append(X_train, add_train, axis=1)
        X_test = np.append(X_test, add_test, axis=1)
    
        print(f"\n\n After Clustering{X_train.shape}")

    elif cluster == 'expec_max':
        rows,columns = X_train.shape
        print(f"\n\n Before Clustering{X_train.shape}")
        clf = mixture.GaussianMixture(n_components=n, covariance_type='full', reg_covar=0.00001)
        clf.fit(X_train)
        add_train = pd.DataFrame(clf.predict(X_train))
        add_test = pd.DataFrame(clf.predict(X_test))
        X_train = np.append(X_train, add_train, axis=1)
        X_test = np.append(X_test, add_test, axis=1)
    ...
    plt.savefig(f"{dataset_to_use}_{cluster}_{dimreduc}_neuralnet_validation.png")

    return best_lr

def get_learning_curve(file_name, dataset_to_use, learning_rate=0.001, dimreduc='', cluster=''):
    i_X_train, i_X_test, y_train, y_test, train_samples_list = helper.get_dataset(dataset_to_use, file_name, is_nn=True)
    ...
    plt.legend(loc="lower right")
    plt.savefig(f"{dataset_to_use}_{cluster}_{dimreduc}_neuralnet_learning.png")
...
if __name__ == "__main__":
    ...
    for alg in ['', 'pca', 'ica', 'rca', 'rfe']:
        lr = get_validation_curve(file_name, dataset_to_use, dimreduc=alg)
        get_learning_curve(file_name, dataset_to_use, learning_rate=lr, dimreduc=alg)

    for cl in ['kmeans', 'expec_max']:
        for alg in ['pca', 'ica', 'rca', 'rfe']:
            lr = get_validation_curve(file_name, dataset_to_use, dimreduc=alg, cluster=cl)
            get_learning_curve(file_name, dataset_to_use, learning_rate=lr, dimreduc=alg, cluster=cl)
